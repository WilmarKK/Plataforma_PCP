"""
Regras de produção para a máquina Furnax.
...
Constantes:
- FURNAX_DEFAULT_SPEED: velocidade padrão de produção
- FURNAX_MICRO_ONDULADO_SPEED: velocidade para micro ondulado
...
FURNAX_DEFAULT_SPEED = 4000
FURNAX_MICRO_ONDULADO_SPEED = 2000
...
SPEED_KEYWORDS = {
    'micro ondulado': FURNAX_MICRO_ONDULADO_SPEED
}

def extract_setup_time(row: Dict[str, Any]) -> int:
    """
    Extrai o tempo de setup para a máquina Furnax com base no campo 'Processo'.
...
def extract_production_speed(row: Dict[str, Any]) -> int:
    """
    Retorna a velocidade padrão de produção da Furnax, considerando micro ondulado.
    :param row: Dicionário com os dados do registro.
    :return: Velocidade padrão (peças/hora).
    """
    return extract_by_keywords(row, 'Processo', SPEED_KEYWORDS, FURNAX_DEFAULT_SPEED)
